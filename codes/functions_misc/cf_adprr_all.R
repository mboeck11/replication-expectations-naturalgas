cf_adprr_all = function(run, nhor, shockVar, restrVar, varNames, Q_store=NULL, verbose=TRUE){
  ...
  for(irep in 1:thindraws){
    ...
    #----------------------------------------------------------#
    # example restrict idx_restr'th variable in system
    ei             = matrix(0, n, 1)
    ei[idx_restr,] = 1
    ...
    #----------------------------------------------------------#
    # allow all shocks to deviate from its unconditional distribution
    Xi1       = diag(n)[idx_shock,,drop=FALSE]
    Xir       = kronecker(diag(nhor-1),Xi1)
    Xihat     = rbind(cbind(Xi1,matrix(0,1,(nhor-1)*n)),
                      cbind(matrix(0,(nhor-1),n),Xir))
    ghat      = matrix(0, nhor, 1)
    Sigghat   = matrix(0, nhor, nhor)
    ghat[1,1] = 1
