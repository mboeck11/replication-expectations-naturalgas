compute_irf <- function(run, nhor, scaling=1, ident="cholesky"){
  ...
  for(irep in 1:thindraws){
    ...
    shock <- shock%*%diag(1/diag(shock))*scaling
    
    impresp1 <- array(NA_real_, c(n, n, nhor))
    impresp1[,1:M,1] <- shock
    compMati <- compMat
    for(ihor in 2:nhor){
      impresp1[,1:M,ihor] <- t(Jm) %*% compMati %*% Jm %*% shock
